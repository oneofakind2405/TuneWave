...
export function CreateEventForm({
  ...
  const [randomLatitude, setRandomLatitude] = useState(0);
  const [randomLongitude, setRandomLongitude] = useState(0);
  ...
  useEffect(() => {
    if (open) {
      const lat = 40.7128 + (Math.random() - 0.5) * 2;
      const lon = -74.0060 + (Math.random() - 0.5) * 2;
      setRandomLatitude(lat);
      setRandomLongitude(lon);
      form.reset({
